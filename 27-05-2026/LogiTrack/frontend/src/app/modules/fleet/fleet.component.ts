...
@Component({
  ...
  template: `
    <div class="fleet-page">
      <div class="page-header">
        ...
        <button class="btn-primary" (click)="showAddVehicle.set(true)">+ Add Vehicle</button>
      </div>
      ...
      <div class="modal-overlay" *ngIf="showAddVehicle()" (click)="showAddVehicle.set(false)">
        <form class="modal" (click)="$event.stopPropagation()" (ngSubmit)="addVehicle()">
          <h3>Add Vehicle</h3>
          <div class="form-grid">
            <label>Name<input name="name" [(ngModel)]="vehicleForm.name" required /></label>
            <label>License Plate<input name="licensePlate" [(ngModel)]="vehicleForm.licensePlate" required /></label>
            <label>Type
              <select name="type" [(ngModel)]="vehicleForm.type">
                <option>Truck</option>
                <option>Van</option>
                <option>SUV</option>
            ...
            <label>Make<input name="make" [(ngModel)]="vehicleForm.make" /></label>
            <label>Model<input name="model" [(ngModel)]="vehicleForm.model" /></label>
            <label>Year<input name="year" type="number" [(ngModel)]="vehicleForm.year" /></label>
            <label>Capacity (tons)<input name="capacity" type="number" [(ngModel)]="vehicleForm.maxLoadCapacity" /></label>
            <label>IoT Device<input name="iot" [(ngModel)]="vehicleForm.iotDeviceId" /></label>
          </div>
          <div class="modal-actions">
            <button type="button" class="btn-secondary" (click)="showAddVehicle.set(false)">Cancel</button>
            <button type="submit" class="btn-primary" [disabled]="savingVehicle()">{{ savingVehicle() ? 'Saving...' : 'Save Vehicle' }}</button>
          </div>
        </form>
      </div>
    </div>
  ...
  styles: [`
    ...
    .modal { width: 560px; background: white; border-radius: 12px; padding: 22px; box-shadow: 0 24px 70px rgba(15,23,42,.25); }
    .modal h3 { margin: 0 0 16px; font-size: 18px; }
    .form-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; }
    .form-grid label { display: flex; flex-direction: column; gap: 6px; font-size: 12px; font-weight: 600; color: #475569; }
    .form-grid input, .form-grid select { border: 1px solid #cbd5e1; border-radius: 8px; padding: 9px 10px; font-size: 14px; }
...
export class FleetComponent implements OnInit, OnDestroy {
  ...
  addVehicle() {
    if (!this.vehicleForm.name || !this.vehicleForm.licensePlate) return;
    this.savingVehicle.set(true);
    this.vehicleApi.create(this.vehicleForm).subscribe({
      next: vehicle => {
        this.vehicles.update(vs => [vehicle, ...vs]);
