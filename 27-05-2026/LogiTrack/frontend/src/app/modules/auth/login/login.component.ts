...
@Component({
    ...
    <div class="login-page">
      <div class="login-card">
        ...
        <div class="login-form">
          <h2>Sign In</h2>
          <div class="auth-tabs">
            <button [class.active]="mode() === 'login'" (click)="mode.set('login')" type="button">Login</button>
            <button [class.active]="mode() === 'register'" (click)="mode.set('register')" type="button">Create Account</button>
          </div>
          ...
          <div class="form-group" *ngIf="mode() === 'register'">
            <label>Name</label>
            <input [(ngModel)]="name" type="text" class="form-input" placeholder="Operations Admin" />
          </div>
          <div class="form-group">
            <label>Password</label>
            <input [(ngModel)]="password" type="password" class="form-input" placeholder="••••••••" (keyup.enter)="submit()" />
          </div>
  ...
  styles: [`
    ...
    .auth-tabs { display: grid; grid-template-columns: 1fr 1fr; background: #f1f5f9; border-radius: 10px; padding: 4px; margin-bottom: 18px; }
    .auth-tabs button { border: 0; background: transparent; border-radius: 8px; padding: 9px; font-weight: 600; color: #64748b; cursor: pointer; }
    .auth-tabs button.active { background: white; color: #1d4ed8; box-shadow: 0 1px 3px rgba(15,23,42,.12); }
...
export class LoginComponent {
  ...
  login() {
    ...
    this.error.set('');
    setTimeout(() => {
      const user = this.getUsers().find(u => u.email.toLowerCase() === this.email.toLowerCase() && u.password === this.password);
      if (user) {
        localStorage.setItem('token', `demo-token-${user.email}`);
        localStorage.setItem('user', JSON.stringify({ email: user.email, name: user.name, role: user.role }));
        this.router.navigate(['/dashboard']);
      } else {
        this.error.set('Invalid email or password. Create an account first if this is a new user.');
  ...
  register() {
    ...
    setTimeout(() => {
      ...
      users.push({ email: this.email, password: this.password, name: this.name, role: 'Admin' });
      localStorage.setItem('logitrack-users', JSON.stringify(users));
      localStorage.setItem('token', `demo-token-${this.email}`);
      localStorage.setItem('user', JSON.stringify({ email: this.email, name: this.name, role: 'Admin' }));
      this.router.navigate(['/dashboard']);
    }, 800);
  }

  private getUsers(): Array<{ email: string; password: string; name: string; role: string }> {
    const stored = localStorage.getItem('logitrack-users');
    const users = stored ? JSON.parse(stored) : [];
    if (!users.some((u: any) => u.email === 'admin@logitrack.com')) {
      users.push({ email: 'admin@logitrack.com', password: 'admin123', name: 'System Admin', role: 'Admin' });
      localStorage.setItem('logitrack-users', JSON.stringify(users));
    }
    return users;
  }
