...
@Component({
  ...
  template: `
    <div class="warehouse-page">
      ...
      <div class="modal-overlay" *ngIf="addingItem()" (click)="addingItem.set(false)">
        <form class="modal wide" (click)="$event.stopPropagation()" (ngSubmit)="addItem()">
          <h4>Add Inventory Item</h4>
          <div class="form-grid">
            <div class="form-group">
              <label>SKU</label>
              <input name="sku" [(ngModel)]="itemForm.sku" class="form-input" required />
            </div>
            <div class="form-group">
              <label>Name</label>
              <input name="name" [(ngModel)]="itemForm.name" class="form-input" required />
            </div>
            <div class="form-group">
              <label>Category</label>
              <input name="category" [(ngModel)]="itemForm.category" class="form-input" required />
            </div>
            <div class="form-group">
              <label>Location</label>
              <input name="location" [(ngModel)]="itemForm.location" class="form-input" required />
            </div>
            <div class="form-group">
              <label>Quantity</label>
              <input name="quantity" type="number" [(ngModel)]="itemForm.quantity" class="form-input" />
            </div>
            <div class="form-group">
              <label>Min Stock</label>
              <input name="minStockLevel" type="number" [(ngModel)]="itemForm.minStockLevel" class="form-input" />
            </div>
            <div class="form-group">
              <label>Unit Weight</label>
              <input name="unitWeight" type="number" [(ngModel)]="itemForm.unitWeight" class="form-input" />
            </div>
            <div class="form-group">
              <label>Barcode</label>
              <input name="barcodeData" [(ngModel)]="itemForm.barcodeData" class="form-input" />
            </div>
          </div>
          <div class="modal-actions">
            <button type="button" class="btn-secondary" (click)="addingItem.set(false)">Cancel</button>
            <button type="submit" class="btn-primary" [disabled]="savingItem()">{{ savingItem() ? 'Saving...' : 'Save Item' }}</button>
          </div>
        </form>
      </div>
    </div>
...
export class WarehouseComponent implements OnInit {
  ...
  addItem() {
    ...
    if (!warehouse || !this.itemForm.sku || !this.itemForm.name) return;
    this.savingItem.set(true);
    this.api.addInventoryItem(warehouse.id, this.itemForm).subscribe({
      next: () => {
  ...
  updateQuantity() {
    ...
    if (!item) return;
    this.api.updateQuantity(item.id, this.newQty).subscribe({
      next: () => {
