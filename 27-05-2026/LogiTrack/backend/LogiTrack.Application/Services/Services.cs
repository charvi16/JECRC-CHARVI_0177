...
public class ShipmentService : IShipmentService
{
    ...
    public async Task<PagedResult<ShipmentDto>> GetShipmentsAsync(int page, int pageSize, string? status = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(status) && Enum.TryParse<ShipmentStatus>(status, true, out var parsedStatus))
        {
            var filtered = await _uow.Shipments.GetByStatusAsync(parsedStatus);
        ...
        {
            shipments = await _uow.Shipments.GetShipmentsWithDetailsAsync(page, pageSize);
        }
